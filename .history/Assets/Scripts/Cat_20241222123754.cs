...
public class Cat : MonoBehaviour
{
    ...
    private Vector3 originalScale; // Store original scale for climbing
    ...
    private void HandleClimbing()
    {
        ...
        {
            // Set exact velocity for climbing
            rb.velocity = new Vector2(0f, verticalInput * moveSpeed);
        }
        else
        {
            // Completely stop when no input
            rb.velocity = Vector2.zero;
            // Ensure position is maintained
            rb.position = new Vector2(ladderXPosition, rb.position.y);
        }

        // Maintain position on ladder
    ...
    private void ToggleClimbing()
    {
        ...
        if (isClimbing)
        {
            ...
            float offsetX = -0.30f; // Use consistent offset regardless of direction
            transform.position = new Vector2(ladderXPosition + offsetX, transform.position.y);
            
            // Freeze rotation and horizontal movement
            rb.constraints = RigidbodyConstraints2D.FreezeRotation | RigidbodyConstraints2D.FreezePositionX;
            
            // Set a fixed scale while climbing (prevent flipping)
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        if (other.CompareTag("Ladder"))
        {
            if (other.gameObject == currentLadder)
            {
                if (isClimbing)
                {
                    ToggleClimbing(); // Properly exit climbing state
                }
                currentLadder = null;
            }
        }
