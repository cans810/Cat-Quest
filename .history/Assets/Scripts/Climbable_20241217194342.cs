...
public class Climbable : MonoBehaviour
{
    ...
    {
        float verticalInput = Input.GetAxisRaw("Vertical"); // Raw input avoids smoothing issues
        float targetClimbingVelocity = verticalInput * climbingSpeed;

        // Smooth transition using Mathf.SmoothDamp
        currentClimbingVelocity = Mathf.SmoothDamp(currentClimbingVelocity, targetClimbingVelocity, ref climbingAcceleration, smoothTime);

        // Maintain player's horizontal velocity while only modifying vertical velocity
