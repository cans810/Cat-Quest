...
public class Cat : MonoBehaviour
{
    ...
    private TilemapCollider2D currentLadder;
    ...
    {
        Vector2 colliderCenter = currentLadder.bounds.center;
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        {
            tavuk chicken = other.GetComponent<tavuk>();
            if (chicken != null)
            {
        ...
        {
            TilemapCollider2D ladder = other.GetComponent<TilemapCollider2D>();
            if (ladder != null)
            {
                // Check if this ladder is closer than the current one
                if (currentLadder == null || IsCloserLadder(ladder))
                {
    ...
    private bool IsCloserLadder(TilemapCollider2D ladder)
    {
        float distanceToNewLadder = Vector2.Distance(transform.position, ladder.bounds.center);
        float distanceToCurrentLadder = Vector2.Distance(transform.position, currentLadder.bounds.center);

        return distanceToNewLadder < distanceToCurrentLadder;
    }
